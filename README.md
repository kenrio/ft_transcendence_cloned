...
Pictionaryに着想を得たリアルタイム・マルチプレイヤーゲーム「お絵描きアイランド」を4人チームで開発。チーム内では主に技術面を担当した。
...
- リアルタイムゲームエンジンの実装：
    - 描画同期（ポイント単位のWebSocket送信、座標補正）
    - チャット・回答判定（大文字小文字を無視した比較）
    - タイマー管理（1秒ごとのブロードキャスト、ラウンド進行制御）
    - スコアリング（メモリ内Mapでのリアルタイム管理 + ラウンド終了時のDBバッチ保存）
...
- WebSocket再接続制御の安定化
    ...
    クライアントの意図しない切断（ネットワーク断・タブ離脱・ページ遷移など）からの再接続処理が大きな課題となった。特にフロントエンドのライフサイクル（コンポーネントのマウント / アンマウント、useEffect の発火タイミング）と WebSocket イベント（onclose / onerror）が競合し、再接続ループ（接続・切断が短時間で繰り返される状態）が発生する問題に直面した。

    対策として、単純な再接続処理ではなく「接続状態を明示的に管理する」方針を取り、接続中・切断中・再接続中といった状態を管理するフラグを導入することで、再接続処理の制御を安定させた。具体的には、コンポーネントのマウント状態を保持する `isMountedRef`、参加処理の重複を防ぐ `hasJoinedRef`、副作用のキャンセルを示す `cancelled` フラグなどを用い、ライフサイクルとWebSocketイベントの両方を考慮した制御を実装した。

    さらに、画面ごと（ロビー、ゲーム準備、ゲーム中、結果）に必要な再接続処理が異なるため、それぞれに適した制御を設計し、チーム内で共有した。例えば、Waiting画面では再接続カウンターをリセットして長時間の待機に対応した一方、Game画面ではゲーム中の意図しないリセットを防ぐためカウンターを保持する設計とした。
